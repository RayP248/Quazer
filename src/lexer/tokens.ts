...
export class Token implements TokenInterface {
  ...
  debug(): void {
    if (this.isOneOfMany(TokenKind.IDENTIFIER, TokenKind.NUMBER, TokenKind.STRING)) {
      console.log(`${tokenKindString(this.kind)} (${this.value}), line: ${this.line}, column: ${this.column}`);
    } else {
      console.log(`${tokenKindString(this.kind)} (), line: ${this.line}, column: ${this.column}`);
    }
  }
...
export function newToken(kind: TokenKind, value: string, line: number, column: number): Token {
  return new Token(kind, value, line, column);
}

export function tokenKindString(kind: TokenKind): string {
  switch (kind) {
    ...
    case TokenKind.OPEN_CURLY: return 'OPEN_CURLY';
    case TokenKind.CLOSE_CURLY: return 'CLOSE_CURLY';
    ...
    case TokenKind.NOT_EQUALS: return 'NOT_EQUALS';
    case TokenKind.LESS: return 'LESS';
    case TokenKind.LESS_EQUALS: return 'LESS_EQUALS';
    case TokenKind.GREATER: return 'GREATER';
    case TokenKind.GREATER_EQUALS: return 'GREATER_EQUALS';
    ...
    case TokenKind.PLUS_EQUALS: return 'PLUS_EQUALS';
    case TokenKind.MINUS_EQUALS: return 'MINUS_EQUALS';
    case TokenKind.SLASH_EQUALS: return 'SLASH_EQUALS';
    case TokenKind.STAR_EQUALS: return 'STAR_EQUALS';
    ...
    default: new Err(new Token(TokenKind.UNKNOWN, '', 0, 0), `Unknown token kind: ${kind}`).throw();
  }
}
